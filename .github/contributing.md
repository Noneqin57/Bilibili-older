# 代码贡献指南
本项目是为了还原 2019 年 12 月 09 日[B站](https://www.bilibili.com/)逐渐弃用的播放页而来。原来只一个小小的用户脚本，后续随着代码规模增大，添加了chrome扩展支持。

**在有限的兼容允许下，使用最新的开发工具，追随最新的前端标准，使用最新的语言特性。** 业余项目，无须束手束脚。
...
---
### 开发  
使用VSCode组织的typescript项目，使用esbuild编译打包为对应chrome扩展和用户脚本。  
项目使用npm本地模块的方式进行了拆分，通常只需要修改src目录下的主模块即可。  
简要流程为：
1. 将项目clone至本地并使用VSCode打开。
2. npm install 或者 npm update。
3. 进行typescript开发。
4. 生成chrome扩展和用户脚本。（详见对应子条目）
5. 测试。（详见对应子条目）
6. git提交。
7. 向master发起合并请求。

#### 扩展
扩展模块位于/chrome目录，主要是负责准备主模块的基础依赖及引导，实际业务本体还是位于/src目录。
扩展模块代码分为三部分：
...
测试：VSCode里运行扩展对应的编译任务（已设为默认任务）会将未打包的扩展程序释放到/dist目录，使用chrome开发者模式的【加载已解压的扩展程序】加载该目录就可以进行测试。

注意：设计background.ts和content.ts的修改需要移除然后重新【加载已解压的扩展程序】才会生效。

#### 用户脚本
用户脚本模块位于/tampermonkey目录，主要是负责引导主模块，实际业务本体还是位于/src目录。运行于脚本管理器提供的上下文，但基本不处理任何业务。  

测试：VSCode里运行用户脚本对应的编译任务会将`main.user.js`生成到/tampermonkey目录下，使用Tampermonkey等脚本管理器加载即可测试。
